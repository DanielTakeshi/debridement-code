...
def servoGo(arm, cx, cy):
    """ 
    Hmm ... in `test.py` we don't return anything?
    """
    regx, regy = pickle.load(open('model-sep.p','rb'))
    aregx, aregy = pickle.load(open('model-angle.p','rb'))
    angle = np.random.choice(np.arange(-80,80,10))

    outx = regx.predict(np.array([cx, cy]))
    outy = regy.predict(np.array([cx, cy]))
    dx = aregx.predict(np.array([np.sin(angle), np.cos(angle), outx, outy, angle]))
    dy = aregy.predict(np.array([np.sin(angle), np.cos(angle), outx, outy, angle]))

    pos = [outx[0]-dx[0], outy[0]-dy[0], -0.158]
...
def collect(arm1, estimate, dx, dy):
    """ 
    Pretty sure `estimate` is the pixel-space x,y and dx,dy are in robot space.
    """
    f = open('data-angle.p', 'a')
    ...
    pickle.dump({'pos': pos, 'rot': rot, 'estimate': estimate, 'del': (dx, dy)}, f)
    f.close()
