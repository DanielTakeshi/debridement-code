...
I used to do this with a fixed height (z-coordinate) but that does not work that well.
...
# NOTE! IMPORTANT! CHANGE THIS! Need to have versions.
...
if __name__ == "__main__":
    ...
    cv2.waitKey(0)
    cv2.imshow("Left Camera BBOX Image", d.left_image_bbox)
    cv2.waitKey(0)
    cv2.imshow("Right Camera Image", d.right_image)
    cv2.waitKey(0)
    cv2.imshow("Right Camera BBOX Image", d.right_image_bbox)
    cv2.waitKey(0)
